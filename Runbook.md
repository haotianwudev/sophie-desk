...
The supervisor rebuilds this unconditionally on every tick, if it's running (see "The
supervisor" below) — any edit anywhere it covers (papers, candidates, gdocs, tasks) gets
picked up, normally within one tick interval (30 min by default). Rebuild by hand if the loop
isn't running, or you just want to be sure right now — safe to re-run, fully replaces the DB
each time:
```bash
...
decision. `Desk.md` itself stays on live Dataview, not this DB — `Papers.md` and
`papers/db-schema/STATUS.md` are what read off it (see the "Local paper index" section
below), so an edit is only reflected there once a tick rebuilds it — automatic while the loop
is running, otherwise rebuild by hand.
