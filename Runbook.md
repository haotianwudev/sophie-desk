...
**Source:** `sophie-pipeline/paper-index/` · powers `Papers.md` and the papers/candidates
backlog. `Desk.md` was tried on this DB too (2026-09-05) and reverted the same day, back to
Dataview — tasks change too often for a rebuild-based board to feel live. **Dataview is still
required, for `Desk.md` only** — don't uninstall it. `Skills.md` never used this DB (or
Dataview): it's fully regenerated by `scripts/gen_skills.py`.

Nothing rebuilds this on a timer (tried via the supervisor, reverted 2026-09-06 — see
`papers/db-schema/DATABASES.md`). A rebuild happens when whatever changed the data triggers
one directly: `scripts/sync_gdocs_index.py` and `scripts/exact_match_gdocs.py` each trigger it
as their own last step (via `sqlite_source_db.trigger_index_rebuild()`, opt out with
`--no-rebuild-index`). A task that changes papers/candidates markdown should do the same as a
closing step (not yet automated). Or just run it by hand any time — safe to re-run, fully
replaces the DB each time:
```bash
...
`pipeline`) with example queries -- and [papers/db-schema/DATABASES.md](papers/db-schema/DATABASES.md)
for the registry of every db file (this one, plus `gdocs/db/gdocs.db`) and how to add a new one.
...
working live. It does not advance any gate, that stays a human decision. It does **not**
rebuild the paper-index DB (tried 2026-09-05/06, reverted — see
`papers/db-schema/DATABASES.md`) — `Desk.md` stays on live Dataview and never touches that DB;
`Papers.md`/`papers/db-schema/STATUS.md` do, and pick up an edit whenever whatever changed the
data triggers a rebuild (see the "Local paper index" section below), not on this loop's tick.
