...
Reads every SKILL.md we can find, pulls its frontmatter, and writes:
  skills/<name>.md   one reference card per skill (standalone; frontmatter is
                     no longer queried live -- Skills.md's two extra tables
                     are computed directly from the same in-memory data this
                     script already has, not a query over skills/*.md)
...
def write_index(skills: list[dict]) -> None:
    ...
    L.append(
        "<!-- computed directly from the same data as the sections above, not a live query -- \n"
        "     this whole file is regenerated by gen_skills.py, so there's nothing to keep in sync. -->\n"
    )
    L.append("### Task/hybrid skills by size\n")
    L.append("| Skill | Kind | Runs as | Lines |")
    L.append("|---|---|---|---|")
    for s in sorted(tasks, key=lambda r: -r["lines"]):
        L.append(f"| [{s['name']}](skills/{s['name']}.md) | {s['kind']} | {s['tier']} | {s['lines']} |")

    L.append("\n### Not shared with agy, by role\n")
    L.append("| Skill | Role | Runs as |")
    L.append("|---|---|---|")
    for s in sorted(orphans, key=lambda r: r["role"]):
        L.append(f"| [{s['name']}](skills/{s['name']}.md) | {s['role']} | {s['tier']} |")
